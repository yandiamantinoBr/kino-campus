...
-- Search freshness: keep exact FTS as the primary path and extend fuzzy matching
-- to the semantic fields used by the client search engine: title, module,
-- category, subcategory and tags. No table/schema contract changes.
  ...
  matched AS (
    ...
    WHERE ranked.search_document @@ v_query
       OR EXISTS (
         ...
         WHERE length(t) >= 4
           AND extensions.word_similarity(t, ranked.fuzzy_text) >= 0.5
       )
  )
  ...
  matched as (
    ...
    join posts_semantic p
     ...
     and (
       ...
       or (length(t.term) >= 4 and extensions.word_similarity(t.term, p.fuzzy_text) >= 0.5)
     )
