-- ============================================================
-- KinoCampus v9.2.1.3 - Presets de data no feed incremental
-- ============================================================
--
-- Objetivo:
--   - Adicionar suporte server-side a datePreset no
--     kc_get_feed_cursor
--   - Preservar semantica temporal por modulo em
--     America/Sao_Paulo
...
CREATE OR REPLACE FUNCTION public.kc_feed_local_date(p_value TIMESTAMPTZ)
RETURNS DATE
LANGUAGE sql
STABLE
SET search_path = ''
...
CREATE OR REPLACE FUNCTION public.kc_feed_event_local_date(p_metadata JSONB, p_created_at TIMESTAMPTZ)
RETURNS DATE
LANGUAGE plpgsql
STABLE
SET search_path = ''
...
DECLARE
  ...
  v_date_preset TEXT := public.kc_feed_normalize_text(COALESCE(p_request_params->>'datePreset', ''));
