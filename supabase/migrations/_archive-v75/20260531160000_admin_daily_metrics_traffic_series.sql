...
-- Revisão profunda /admin/ (rodada 2): amplia o catálogo de séries diárias do
-- gráfico "Pulso diário" com 3 novas variáveis (inspiradas em analytics de
-- comunidades/redes sociais — famílias Tráfego/Alcance/Engajamento):
--   post_views_count     (public.post_view_events)           — Alcance
--   comment_likes_count  (public.comment_likes)              — Engajamento
--   sessions_count       (distinct session_id/dia em         — Tráfego/Acessos
--                         post_view_events ∪ search_queries)
--
-- Mantém o padrão de hardening vigente: wrapper público (INVOKER, search_path='')
-- → kc_private (SECURITY DEFINER, search_path='public'). Como o tipo de retorno
-- muda, as duas funções são recriadas e os privilégios reaplicados.
...
as $$
  ...
  sessions as (
    select day, count(distinct session_id)::bigint as total
    from (
      select date_trunc('day', created_at)::date as day, session_id
      from public.post_view_events
      where created_at >= (select start_day::timestamptz from bounds) and session_id is not null
      union all
      select date_trunc('day', created_at)::date as day, session_id
      from public.search_queries
      where created_at >= (select start_day::timestamptz from bounds) and session_id is not null
  ...
  select
    ...
    coalesce(vw.total, 0) as post_views_count,
    coalesce(cl.total, 0) as comment_likes_count,
    coalesce(se.total, 0) as sessions_count
  from calendar c
  ...
  left join views vw    on vw.day = c.day
  left join clikes cl   on cl.day = c.day
  left join sessions se on se.day = c.day
  order by c.day asc;
$$;
...
comment on function public.kc_admin_dashboard_daily_metrics(timestamptz) is
  'Buckets diários do Dashboard Admin (11 séries): posts, comentários, buscas, votos, ações admin, salvamentos, denúncias, cadastros, visualizações, curtidas em comentários e sessões ativas. Wrapper INVOKER (search_path='''') -> kc_private (SECURITY DEFINER).';
