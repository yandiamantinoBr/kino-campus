...
AS $$
  ...
  ad_events AS (
    SELECT date_trunc('day', created_at)::date AS day,
           count(*) FILTER (WHERE event_name = 'ad_impression')::bigint AS impressions,
           count(*) FILTER (WHERE event_name = 'ad_click')::bigint AS clicks
    FROM public.privacy_analytics_events
    WHERE created_at >= (SELECT start_day::timestamptz FROM bounds)
      AND entity_type = 'ad_campaign'
      AND event_name IN ('ad_impression', 'ad_click')
  ...
  LEFT JOIN ad_events ad ON ad.day = c.day
  ORDER BY c.day ASC;
$$;
...
AS $$
  ...
  WHERE public.kc_is_admin(auth.uid())
$$;
