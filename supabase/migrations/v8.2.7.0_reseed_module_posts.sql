-- ============================================================
-- v8.2.7.0 — Re-seed forçado dos 44 posts de módulos
--
-- Problema resolvido:
--   A migration v8.2.6.0 só inseria os posts seed se post_count = 0.
--   Se o banco já tinha qualquer post quando foi aplicada, os dados
--   demo não foram inseridos → módulos (caronas, moradia, eventos,
--   oportunidades, achados-perdidos, compra-venda) ficavam sem posts.
--
-- Esta migration usa INSERT ... ON CONFLICT (legacy_id) DO UPDATE
--   → posts novos: inseridos
--   → posts existentes com legacy_id: têm module/status/category atualizados
--   → posts sem legacy_id (criados pelo usuário): NÃO são afetados
-- ============================================================

-- 1) Garante coluna status (idempotente)
alter table public.posts
  add column if not exists status text not null default 'published';

-- 2) Corrige posts existentes com status NULL
update public.posts
...
insert into public.posts
  (legacy_id, author_id, title, description, price, module, category, status, votos, metadata)
values
...
from public.posts
where module in ('caronas','moradia','eventos','oportunidades','achados-perdidos','compra-venda','livros')
group by module
order by module;
