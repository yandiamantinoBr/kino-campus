-- Reconcile Cadu review readiness probe with INVOKER public admin helpers.
-- After 20260805120000, public.kc_is_admin/operator are SECURITY INVOKER facades.
-- RLS policies may reference kc_private.kc_is_admin_impl because Postgres rewrites
-- policy deps when a function is renamed/moved.
...
AS $function$
  with objects as (
    select
      ...
      pg_catalog.to_regprocedure('kc_private.kc_is_admin_impl(uuid)') as is_admin_impl_rpc,
      pg_catalog.to_regprocedure('kc_private.kc_is_operator_impl(uuid)') as is_operator_impl_rpc,
      pg_catalog.to_regprocedure(
    ...
    select
      ...
      and exists (
        ...
        where policy_row.polrelid = objects.review_table
          ...
          and pg_catalog.pg_get_expr(
            ...
            'kc_private.kc_is_admin_impl(( SELECT auth.uid() AS uid))'
          )
      )
      and exists (
        select 1
        ...
        where policy_row.polrelid = objects.review_table
          ...
          and pg_catalog.pg_get_expr(
            policy_row.polqual, policy_row.polrelid
          ) in ('public.kc_is_current_session_active()', 'kc_is_current_session_active()')
          and pg_catalog.pg_get_expr(
      ...
      and exists (
        ...
        where function_row.oid = objects.is_admin_rpc
          ...
          and pg_catalog.md5(function_row.prosrc) = '2819cf19c09be4fa80c12280003fb323'
      ...
      and exists (
        ...
        join pg_catalog.pg_language as language_row
          on language_row.oid = function_row.prolang
        where function_row.oid = objects.is_admin_impl_rpc
          and function_row.prokind = 'f'
          and function_row.pronargs = 1
          and function_row.proargnames[1:1] = array['p_user_id']::text[]
          and function_row.prorettype = 'pg_catalog.bool'::regtype
          ...
          and function_row.proconfig = array['search_path=""']
          and language_row.lanname = 'plpgsql'
          and pg_catalog.md5(function_row.prosrc) = '8f7bb23270c1f080d591b353c2ab4ab5'
          and pg_catalog.has_function_privilege('anon', function_row.oid, 'execute')
          and pg_catalog.has_function_privilege('authenticated', function_row.oid, 'execute')
          and pg_catalog.has_function_privilege('service_role', function_row.oid, 'execute')
      )
      and exists (
        ...
        where function_row.oid = objects.is_operator_rpc
          ...
          and function_row.proconfig = array['search_path=""']
          and language_row.lanname = 'sql'
          and pg_catalog.md5(function_row.prosrc) = '5d900edec20e6808378f2aaa9e996453'
          and pg_catalog.has_function_privilege('anon', function_row.oid, 'execute')
          and pg_catalog.has_function_privilege('authenticated', function_row.oid, 'execute')
          and pg_catalog.has_function_privilege('service_role', function_row.oid, 'execute')
          and (
      ...
      and exists (
        ...
        join pg_catalog.pg_language as language_row
          on language_row.oid = function_row.prolang
        where function_row.oid = objects.is_operator_impl_rpc
          and function_row.prokind = 'f'
          and function_row.pronargs = 1
          and function_row.proargnames[1:1] = array['p_user_id']::text[]
          and function_row.prorettype = 'pg_catalog.bool'::regtype
      ...
      and not exists (
        ...
        where function_row.oid = any(array[
           ...
           objects.is_admin_impl_rpc::oid,
           objects.is_operator_rpc::oid,
           objects.is_operator_impl_rpc::oid,
