...
| `kc-privacy-pending-payload-v1` | `sessionStorage` | Conserva temporariamente, na aba atual, o formulário LGPD preenchido por visitante durante o login. Pode conter e-mail, mensagem e metadados informados pela própria pessoa; não entra em URL, log ou `localStorage`. | Funcional necessário para não perder o pedido durante a autenticação | Até 15 minutos, envio confirmado ou fechamento da aba | Central de Ajuda; não é exibido no admin enquanto não houver envio confirmado |
| `kc_help_form_draft_v1` | `sessionStorage` | Rascunho do formulário da Central de Ajuda, incluindo campos que podem conter PII, isolado à aba e à conta ativa. A antiga cópia em `localStorage` é removida sem ser restaurada. | Funcional | Até 7 dias em uma aba mantida aberta, envio/reset explícito ou fechamento da aba | Central de Ajuda; não representa ticket nem protocolo até o envio |
