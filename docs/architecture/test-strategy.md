...
**Versão:** v76.35.0 · **Atualizado em:** 2026-06-20

> Documenta as 184 suites Jest + 10 specs Playwright: o que cada uma cobre,
...
4. [Diretório integration/ — 127 suites](#4-diretório-integration--127-suites)
...
| **Execução rápida** | Todos os 184 suites Jest rodam sem network, sem browser e sem Supabase real |
...
| `tests/integration/` | **127** | Controllers, adapters, sub-módulos KCAPI e parser de busca |
...
| **Total** | **194** | (184 Jest + 10 Playwright specs) |

### Contagem canônica

```
Jest: 184 suites · 3732 testes
Playwright: 10 specs · 68 testes listados
```

**Regra imutável:** `npm test` DEVE sempre retornar `≥184 passed, 184 total` e `≥3732 passed, 3732 total`.
...
A tabela abaixo é agrupada por domínio e não lista todos os 126 arquivos individualmente.
...
### Sub-grupo: KCAPI sub-módulos (18 suites)

| Suite | Sub-módulo | O que cobre |
|-------|-----------|------------|
| `kc-api-auth-module.test.js` | `kc-api.auth.js` | `signIn`, `signUp`, `getCurrentUser`, `logout` |
| `kc-api-authors-module.test.js` | `kc-api.authors.js` | `MOCK_USERS`, aliases publicos, resolucao legado e ordem HTML |
| `kc-api-client.test.js` | `kc-api.client.js` | Fachada principal: `registerAdapter`, `getActiveDriver`, diagnósticos |
| `kc-api-normalize-post-snapshot.test.js` | `KCAPI.normalizePost` | Snapshots pre-extracao de aliases, datas, autor legado, midia e regra `compra-venda` |
| `kc-api-posts-normalize-module.test.js` | `kc-api.posts-normalize.js` | Delegacao de `normalizePost`, helper `pickFirstNonEmpty` e ordem HTML |
...
| `kc-api-adapter-registry-contract.test.js` | Registro, seleção, fallback e fail-fast dos adapters |
