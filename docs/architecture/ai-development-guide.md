...
**Versão:** v63.0.0 · **Atualizado em:** 2026-04-30
...
| Campo | Valor |
|-------|-------|
| Branch principal | `kinocampus-V63.0-foundations` |
| Branch de features | `feature/v63.X.Y-descricao-curta` |
| appVersion | `63.0.0` (v63 encerrada; patch PUBLIC-A11Y pontual documentado em `docs/qa/reports/`) |
| frontendRuntimeVersion | `8.6.0` (constante canônica — **nunca alterar**) |
| Jest | 135 suites · 3064 testes |
...
```
1. git checkout kinocampus-V63.0-foundations
2. git pull
3. git checkout -b feature/v63.X.Y-descricao-curta
4. [ implementar mudanças ]
5. npm run check:all          ← DEVE ser 5/5 verdes
   npm test                   ← DEVE ser ≥135/135 suites, ≥3064/3064 testes
6. git add <arquivos específicos>
7. git commit -m "tipo(escopo): descrição\n\nCo-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>"
8. git push -u origin feature/v63.X.Y-descricao-curta
9. gh pr create --title "..." --body "..."
10. gh pr merge <número> --squash --delete-branch
11. git checkout kinocampus-V63.0-foundations
12. git pull
```
...
```
feature/v63.X.Y-descricao-curta

Exemplos válidos:
  feature/v63.1.0-public-a11y-post-card-comments
  feature/v63.2.0-public-a11y-card-labels
  feature/v63.3.0-public-a11y-rendered-content

Proibido:
  main, master, develop, fix/..., hotfix/...
  Qualquer nome sem o prefixo feature/v63.X.Y-
```
...
- `branch` é exatamente `"kinocampus-V63.0-foundations"`
...
- Antes de commitar: `npm test` deve mostrar `≥135 passed, 135 total` e `≥3064 passed, 3064 total`
...
arquivos canônicos. Validator agora verifica 156 itens (baseline v63).
...
| Push direto para `kinocampus-V63.0-foundations` | Branch protegida — fluxo via PR |
...
| Reduzir contagem de testes sem aprovação explícita | Contagem mínima é 135/3064 |
...
| Estado atual da release v63, iterações, DoD | `RELATORIO-KINOCAMPUS-v63.md` |
...
# Testes
npm test                 # 135 suites · 3064 testes
