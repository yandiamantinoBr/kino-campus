...
Trabalho iniciado em 2026-08-31 às 17:04 UTC. O lote principal foi integrado pelas [PR #922](https://github.com/yandiamantinoBr/kino-campus/pull/922) e [PR #923](https://github.com/yandiamantinoBr/kino-campus/pull/923), após CI verde. O domínio canônico foi conferido no SHA `e6c3e82d227aa1fa8bfde60df71868b2d1adb649`; não apenas o preview. O complemento de toque e estes relatórios pertencem à [PR #924](https://github.com/yandiamantinoBr/kino-campus/pull/924), cujo histórico registra a integração e a verificação pós-merge do próprio complemento.
...
- E2E completo final no código-fonte: **307/307 aprovados**, em 6,7 minutos, sem skips/falhas/flaky, após incluir a regressão de hover. A rodada anterior, após corrigir o rascunho, havia passado 300/300 em 9,4 minutos. O primeiro baseline do lote havia passado 295/296, com a falha investigada acima; não foi removido nem afrouxado o teste.
...
- Artefato final do complemento: **20/20 Chrome/Edge**, zero flaky, 47,9 segundos, cobrindo toque, layout, teclado, busca, notificações, login e chat. Evidências `s24-release-source-e2e.json` e `s24-release-artifact-e2e.json`.
...
O build local do complemento detectou um segundo hardlink temporário do Google Drive no `vercel.json` de origem e recusou corretamente esse input. Nenhum link/configuração foi editado, o Drive não foi interrompido e as guardas não foram relaxadas. Uma cópia mínima dos arquivos públicos foi criada fora da pasta sincronizada; o build normal, preconnect e cache passaram com links únicos e revisão `6267f0a8`. Os 266 inputs públicos produziram 259 arquivos de artefato; nenhum `.env`, diretório privado ou `node_modules` foi copiado. O `dist` anterior foi preservado de forma recuperável, e a origem não sofreu alterações. A documentação posterior não altera nenhum input público desse build. Proveniência: `s24-isolated-artifact-provenance.json`.

O registro final do SHA de merge/deploy, comparação dos assets no domínio, QA pública posterior e sincronização da main fica no [comentário permanente de fechamento da PR #924](https://github.com/yandiamantinoBr/kino-campus/pull/924#issuecomment-5483571613). Esta separação evita que a mera inclusão do próprio SHA num documento gere um novo deploy com outro SHA. Os números de performance acima continuam vinculados às revisões e execuções explicitamente indicadas, não são atribuídos a um deploy posterior sem medição.
