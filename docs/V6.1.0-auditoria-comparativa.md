...
- **Anúncios em `product.html?id=...` não carregavam** em alguns cenários porque o arquivo de banco local estava em `assets/data/database.json` mas algumas páginas/trechos buscavam `data/database.json`.
- Correção: **o banco oficial continua em `assets/data/database.json`** e existe um **alias** em `data/database.json` (compatibilidade retroativa).
