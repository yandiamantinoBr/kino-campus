...
- **Controle válido** (snapshot validado e dentro do prazo,
  ...
  **Executar**/**Executar real** (play; vermelho quando o estágio altera
  dados reais, e "Executar real" sempre sujeito à aprovação Ed25519).
- **Controle expirado** (sessão administrativa vencida ou snapshot fora do
  ...
  `fa-shield-halved`). O clique NÃO executa nada: apenas renova o snapshot e
  a verificação prévia; com a sessão expirada o handler falha fechado com
