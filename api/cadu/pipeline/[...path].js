// KinoCampus — proxy Vercel: /api/cadu/pipeline/* → cadu-api (VPS)
// Catch-all: passa path inteiro + método + body pra cadu-api.
//
// Endpoints cobertos:
//   GET  /api/cadu/pipeline              (lista stages + active + history)
//   GET  /api/cadu/pipeline/runs         (histórico)
//   POST /api/cadu/pipeline/run          (dispara run)
//   GET  /api/cadu/pipeline/:id          (status)
//   POST /api/cadu/pipeline/:id/stop     (mata subprocess)
//
// IMPORTANTE: SSE (GET /api/cadu/pipeline/:id/stream) NÃO funciona em Vercel
// serverless (timeout 10-60s). O cliente (admin/cadu.html) faz SSE diretamente
// pro cadu-api via Traefik. Ver getStreamUrl() no admin-cadu.controller.js.

export default async function handler(req, res) {
  // CORS
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.setHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
  res.setHeader("Access-Control-Allow-Headers", "Authorization, Content-Type");
  if (req.method === "OPTIONS") return res.status(204).end();

  const CADU_API_URL = process.env.CADU_API_URL || "";
  const CADU_API_TOKEN = process.env.CADU_API_TOKEN || "";
  if (!CADU_API_URL || !CADU_API_TOKEN) {
    return res.status(503).json({ ok: false, error: "CADU_API_URL/TOKEN not configured" });
  }

  // Em Vercel catch-all, req.query.path é um array com os segmentos depois de /api/cadu/pipeline/
  // Ex: /api/cadu/pipeline/runs → req.query.path = ["runs"]
  //     /api/cadu/pipeline/abc/stop → req.query.path = ["abc", "stop"]
  const subPath = Array.isArray(req.query.path) ? req.query.path.join("/") : "";
  const targetUrl = `${CADU_API_URL.replace(/\/$/, "")}/api/pipeline${subPath ? "/" + subPath : ""}`;

  try {
    const upstream = await fetch(targetUrl, {
      method: req.method,
      headers: {
        Authorization: `Bearer ${CADU_API_TOKEN}`,
        "Content-Type": "application/json",
        Accept: "application/json",
      },
      body: req.method !== "GET" && req.method !== "HEAD" ? JSON.stringify(req.body || {}) : undefined,
    });

    const ct = upstream.headers.get("content-type") || "application/json";
    const body = await upstream.text();
    res.status(upstream.status).setHeader("Content-Type", ct).send(body);
  } catch (e) {
    res.status(502).json({ ok: false, error: `Upstream unreachable: ${e.message}` });
  }
}