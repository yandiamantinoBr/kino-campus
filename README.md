...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.7.2` concluída com i18n runtime fase 3: `assets/js/kc-i18n.js` (`803L`, `41 693` bytes) agora expõe `applyTooltips(root)`, o dicionário ganhou `28` chaves `tooltip.*` cobrindo tooltips de tema, ranking, editor de rich-text, filtros admin e badges ODS; os `22` HTMLs canônicos declaram `55` marcações `data-i18n-tooltip` em toda tag com `title` estático nao-vazio; `scripts/hygiene-check.js` valida o gate declarativo da fase 3; baseline expandida para **124/124 suites · 2546/2546 testes**; próxima iteração: `v12.7.3` (gate i18n B2 / locale switcher).
...
| Testes | Jest: 124 suites de regressão e contrato |
