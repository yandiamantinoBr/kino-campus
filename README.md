...
**Status atual:** v11 executada até `v11.31.6`, com a rodada principal encerrada no release gate `v11.23.0`, trilha i18n concluída em `v11.24.x`, baseline de testes elevada para 86/86 suites e 1598/1598 testes em `v11.31.6`, trilha iOS/Safari encerrada (6/6 issues), trilha de paridade de controllers encerrada (v11.28.x), trilha SWR residual concluída (`profile` + `my-posts`), split do monolito `supabase.adapter.js` concluído — 10/10 grupos extraídos para sub-adapters via `window._KCSA`; `supabase.adapter.js` reduzido de 4041L para 420L (−3621L acumulado), trilha `v11.30.x` encerrada com o split de `product.controller.js` estabilizado, e a trilha `v11.31.x` avançada com a quinta extração estrutural de `kc-create-post.js`: `kcHandleCreateSubmit` (pipeline de submit/edição, 707L) externalizado para `kc-create-post.submit.js` via `window._KCCreatePost.submit`; núcleo reduzido a 1260L.
...
| Testes | Jest: 86 suites de regressão e contrato |
