...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.8.1` concluída com correções a11y estruturais WCAG 2.1 AA nos `22` HTMLs: CSS `kc-sr-only`, `<h1 class="kc-sr-only">` em `9` páginas sem heading, carousel `index.html` h1→h2, skip link + `id="kc-main"` em `21` páginas, `aria-label` nos `<nav>` de todas as páginas, `3` selects admin com label, `2` botões icon-only com `aria-label`, `1` label com `for`; `6` chaves novas no dicionário (`446` total); `runA11yStructureChecks()` no hygiene; `+10` testes em `tests/a11y.test.js`; baseline expandida para **125/125 suites · 2572/2572 testes**; próxima iteração: `v12.9.x` (trilha B4 — Playwright E2E scaffold).
...
| Testes | Jest: 125 suites de regressão e contrato |
