...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.4.6` concluída com o sexto split funcional do adapter local: criado `assets/js/adapters/local.profile.adapter.js` com namespace `window._KCLA.profile` (**157L**, **4 exports**) para perfil/avatar (`readProfile`, `getMyProfile`, `updateMyProfile`, `uploadProfileAvatar`); `assets/js/adapters/local.adapter.js` caiu de `1031L` para **`850L`** (`-181L`, `38 582` bytes) via wrappers finos, `getLocalProfileModule()`, `buildLocalProfileDeps()` e `readLocalProfileSnapshot()`; `22` HTMLs e `8` bootstraps de teste diretos atualizados; nova suíte `tests/local-profile.adapter.test.js` (**25 testes**); baseline expandida para **115/115 suites · 2408/2408 testes**; próxima iteração: `v12.4.7` (split `window._KCLA.help`).
...
| Testes | Jest: 115 suites de regressão e contrato |
...
- iteração encerrada: `v12.4.6` — **split `window._KCLA.profile` concluído** (`local.profile.adapter.js` com `157L`, `4` exports; `local.adapter.js` em `850L` / `38 582` bytes; `22` HTMLs + `8` bootstraps diretos atualizados)
...
- regressão: `115/115` suites, `2408/2408` testes verdes, hygiene `8.6.0`
...
- sub-adapters `window._KCLA.*` operacionais: `notifications`, `ratings`, `saved`, `postsRead`, `postsWrite`, `profile` (6 total)
...
- `local.adapter.js`: após `v12.4.6`, o core caiu de `1862L` / `75 712` bytes para `850L` / `38 582` bytes; `assets/js/adapters/local.notifications.adapter.js` segue com `250L`, `14` exports, `assets/js/adapters/local.ratings.adapter.js` com `339L`, `6` exports, `assets/js/adapters/local.saved.adapter.js` com `252L`, `7` exports, `assets/js/adapters/local.posts-read.adapter.js` com `687L`, `8` exports, `assets/js/adapters/local.posts-write.adapter.js` com `300L`, `7` exports, e `assets/js/adapters/local.profile.adapter.js` entrou com `157L`, `4` exports; os `22` HTMLs consumidores e `8` bootstraps diretos de teste já carregam a cadeia `notifications -> ratings -> saved -> posts-read -> posts-write -> profile -> local.adapter.js`
- próxima iteração: `v12.4.7` — split `window._KCLA.help`
