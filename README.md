...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.4.2` concluída com o segundo split funcional do adapter local: criado `assets/js/adapters/local.ratings.adapter.js` com namespace `window._KCLA.ratings` (**339L**, **6 exports**) para summaries, state, list/upsert e enrich de avaliações; `assets/js/adapters/local.adapter.js` caiu de `1780L` para **`1570L`** (`-210L`, `64 505` bytes) via wrappers finos e `buildLocalRatingsDeps()`; `22` HTMLs e `4` bootstraps de teste diretos atualizados; nova suíte `tests/local-ratings.adapter.test.js` (**23 testes**); baseline expandida para **111/111 suites · 2315/2315 testes**; próxima iteração: `v12.4.3` (split `window._KCLA.saved`).
...
| Testes | Jest: 111 suites de regressão e contrato |
...
- iteração encerrada: `v12.4.2` — **split `window._KCLA.ratings` concluído** (`local.ratings.adapter.js` com `339L`, `6` exports; `local.adapter.js` em `1570L` / `64 505` bytes; `22` HTMLs + `4` bootstraps diretos atualizados)
...
- regressão: `111/111` suites, `2315/2315` testes verdes, hygiene `8.6.0`
...
- sub-adapters `window._KCLA.*` operacionais: `notifications`, `ratings` (2 total)
...
- `local.adapter.js`: após `v12.4.2`, o core caiu de `1862L` / `75 712` bytes para `1570L` / `64 505` bytes; `assets/js/adapters/local.notifications.adapter.js` segue com `250L`, `14` exports, e `assets/js/adapters/local.ratings.adapter.js` entrou com `339L`, `6` exports; os `22` HTMLs consumidores e `4` bootstraps diretos de teste já carregam a cadeia `notifications -> ratings -> local.adapter.js`
- próxima iteração: `v12.4.3` — split `window._KCLA.saved`
