...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.5.2` concluída com o split `window._KCPR.collections`: criado `assets/js/controllers/profile.collections.js` (`556L`, `25 241` bytes, `11` exports), `assets/js/controllers/profile.controller.js` reduzido de **`1261L`** / `48 514` bytes para **`796L`** / `34 426` bytes com `buildCollectionsDeps()` + wrappers finos para posts/comments/saved/activities/tabs, `profile.html` sincronizado com a cadeia `profile.presentation.js -> profile.collections.js -> profile.controller.js` e nova suíte `tests/profile.collections.test.js` (19 testes); baseline expandida para **118/118 suites · 2462/2462 testes**; proxima iteracao: `v12.5.3` (split `window._KCPR.ratings`).
...
| Testes | Jest: 118 suites de regressão e contrato |
...
- iteração encerrada: `v12.5.2` — **split `window._KCPR.collections` concluido** (`profile.collections.js` com `556L`, `25 241` bytes e `11` exports; `profile.controller.js` reduzido para `796L` / `34 426` bytes)
...
- regressão: `118/118` suites, `2462/2462` testes verdes, hygiene `8.6.0`
...
- `assets/js/controllers/profile.controller.js`: apos `v12.5.2`, o core caiu de `1463L` / `56 497` bytes para `796L` / `34 426` bytes; `assets/js/controllers/profile.presentation.js` concentra `28` exports em `518L` / `20 846` bytes e `assets/js/controllers/profile.collections.js` concentra `11` exports em `556L` / `25 241` bytes, preservando `window.KCProfileRefresh`
- próxima iteração: `v12.5.3` — split `window._KCPR.ratings`
