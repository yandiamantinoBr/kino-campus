...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.11.0` concluída: **trilha C1 Service Worker** — `sw.js` (cache-first para shell + passthrough Supabase/CDNs) + `kc-sw-register.js` (IIFE com kill-switch `KCFF.isEnabled('sw.enabled')` padrão `false`) + `tests/sw.test.js` (39 testes) + 22 HTMLs injetados; Jest 126/126 · 2611/2611 · Playwright 51/51 preservados; próxima iteração: `v12.12.0` (trilha C2 — Error boundary + telemetria cliente).
