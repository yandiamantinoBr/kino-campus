...
**Status atual:** v11 executada ate `v11.27.1`, com a rodada principal encerrada no release gate `v11.23.0` (regressao verde, hygiene canonico `8.6.0`, smoke remoto e residuals documentados), trilha i18n concluida em `v11.24.x`, baseline de testes elevado para 59/59 suites e 706/706 testes em `v11.26.x` e trilha iOS/Safari em execucao — auditoria (`v11.27.0`) + 3 fixes CSS aplicados (`v11.27.1`: webkit prefix no hero-pill, font-size:1rem no search input, webkit none no account-setup).
...
| Fase | Entrega | PRs |
|------|---------|-----|
| v11.27.1 | 3 fixes CSS iOS/Safari: A1 `-webkit-backdrop-filter: blur(10px)` adicionado em `.kc-hero-pill` (`styles.css:656`); A2 `font-size: 12px` → `1rem` em `.kc-search-bar input` @media 420px (elimina zoom automatico iOS); B4 `-webkit-backdrop-filter: none` adicionado em reset account-setup (`kc-public-shell.css:126`) | a preencher |
