...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); **v12 ENCERRADA** (*Consolidação & Qualidade Sistêmica* — v12.0.0–v12.13.0) — 13 iterações, 23 sub-módulos/adapters criados, −5600L acumuladas nos hotspots primários, 51 E2E Playwright, Lighthouse CI, Service Worker + telemetria cliente atrás de flags; Jest **127/127 · 2647/2647** · Playwright **51/51** · hygiene **8.6.0 ✓**; próxima fase: **v13** (JS > 1100L remanescentes + Lighthouse produção + KCFF opt-ins).
...
| Fase | Entrega | PRs |
|------|---------|-----|
| v12.13.0 | **Release gate final v12** (docs-only): `CHANGELOG.md` entrada formal `## [12.0.0] - 2026-04-25` com sumário 13 iterações + tabela métricas; `RELATORIO-KINOCAMPUS-V12.md` seção 7 DoD preenchida + §8.43 + cabeçalho "v12 ENCERRADA"; `README.md` status encerramento; baseline inalterada Jest 127/127 · 2647/2647 · Playwright 51/51 | — |
| v12.12.0 | **Trilha C2 — Error boundary + telemetria**: `assets/js/kc-telemetry.js` (IIFE; `window._KCT` com `errors[]` buffer circular máx 50, `push/getErrors/clear/flush`; handler `window.onerror` com preservação `_prevOnError`; listener `unhandledrejection`; listener `beforeunload` para flush final via `sendBeacon`; endpoint via `KC_ENV.telemetryEndpoint`; kill-switch `KCFF.isEnabled('telemetry.enabled')` padrão `false`); `tests/telemetry.test.js` (`36` testes: 14 integridade, 22 cadeia HTML); `22` HTMLs com `kc-telemetry.js` após `kc-sw-register.js`; Jest 127/127 · 2647/2647 · Playwright 51/51 | — |
| v12.11.0 | **Trilha C1 — Service Worker**: `sw.js` (root; `CACHE_VERSION='kc-shell-v12.11.0'`; cache-first para 12 shell assets — `/`, 2 CSS, 9 JS core; passthrough para Supabase/jsDelivr/Google Fonts/FontAwesome; `skipWaiting` + `clients.claim`); `assets/js/kc-sw-register.js` (IIFE com 3 guards: `serviceWorker in navigator`, `typeof KCFF`, `KCFF.isEnabled('sw.enabled')` — padrão `false`; registra `/sw.js` no evento `load`); `tests/sw.test.js` (`39` testes: 10 integridade sw.js, 7 contrato kc-sw-register, 22 cadeia HTML); `22` HTMLs editados com tag `kc-sw-register.js` após `kc-feature-flags.js`; Jest 126/126 · 2611/2611 · Playwright 51/51 preservados | — |
| v12.10.0 | **Trilha B5 — Lighthouse CI**: `.lighthouserc.js` (audita `4` páginas: `/`, `/compra-venda-feed.html`, `/_product.html`, `/admin/index.html`; thresholds warn: perf ≥0.70, a11y ≥0.80, bp ≥0.60, seo ≥0.90); `.github/workflows/lighthouse-ci.yml` (trigger em PRs para `kinocampus-V11.0-foundations`, Ubuntu, `npx lhci autorun`); `@lhci/cli` em devDependencies; script `lhci`; baseline local documentada: index (perf 74, a11y 86, bp 64, seo 100), compra-venda-feed (100/86/64/100); Best Practices penalizado por ausência de HTTPS no local (~10pts, Vercel prod ~85); Jest 125/125 · 2572/2572 · Playwright 51/51 preservados | — |
...
| v12.8.0 | **a11y audit estrutural (docs-only)**: `docs/a11y-audit-v12.8.md` mapeia `7` problemas WCAG 2.1 AA nos `22` HTMLs canônicos — A1: `h1` ausente em `10` páginas (feeds + settings + create-post + my-posts), iniciando hierarchy em `<h2>`; A2: `3x h1` no carousel do `index.html` (deveriam ser `h2`); A3: skip link `<a href="#kc-main">` ausente em `21` páginas (só `index.html` tem) e `<main>` sem `id` em `21` páginas; A4: os dois `<nav>` públicos (`kc-nav-links`, `kc-mobile-nav`) sem `aria-label` em `17` páginas; A5: `3` selects em `admin/moderation.html` sem `aria-label` nem `<label for="">`; A6: `2` botões icon-only com `title` mas sem `aria-label` (`kc-ranking-info-btn` em `index.html` e `admin/index.html`); A7: `<label>Status</label>` sem `for` em `admin/banners.html`; plano de correção detalha `6` chaves novas no dicionário pt-BR, CSS `kc-sr-only` e gate `runA11yStructureChecks()` no hygiene; baseline preservada em **125/125 suites · 2562/2562 testes** | — |
| v12.7.3 | **gate formal da trilha B2 i18n**: `scripts/hygiene-check.js` ganhou `runI18nB2GateChecks()` validando que `kc-i18n.js` mantém `>= 440` chaves únicas e `>= 800` linhas, e que os `22` HTMLs canônicos mantêm `>= 189` marcações `data-i18n-aria-label`, `>= 59` marcações `data-i18n-placeholder`, `>= 55` marcações `data-i18n-tooltip` e `>= 5` marcações `data-i18n-alt` — impedindo regressão silenciosa de qualquer camada da trilha B2; criada `tests/i18n-b2-gate.test.js` com **16 testes** em 3 grupos (integridade do módulo: linhas + chaves + contrato de 9 métodos + `keys()`; totais de markings: os 22 HTMLs listados + 4 superfícies; infraestrutura no código-fonte: `translateWithFallback`, `applyRuntimeI18n`, os 5 helpers, exports e namespaces de runtime); criada `docs/i18n-b2-coverage-v12.7.md` com auditoria completa da trilha B2 (`440` chaves em `18` namespaces, `352` marcações em `5` superfícies declarativas, thresholds de regressão e referências às 5 suítes de teste da trilha); a trilha B2 está **formalmente encerrada**; baseline expandida para **125/125 suites · 2562/2562 testes** | — |
| v12.7.2 | **i18n runtime fase 3 (`title` / tooltips)**: `assets/js/kc-i18n.js` cresce de `732L` → `803L` e adiciona `28` chaves `tooltip.*` cobrindo tooltips de tema (`tooltip.theme-toggle`), ranking (`tooltip.how-it-works`, `tooltip.how-ranking-works`), editor de rich-text (`tooltip.format-bold/italic/underline/strike/code/quote/bullet/link`), filtros admin (`tooltip.filter-status/category/urgency/report-status/reason/module`), controles admin (`tooltip.period-filter`, `tooltip.refresh`, `tooltip.remove-global-limit`, `tooltip.color-start/end`), perfil (`tooltip.view-author`, `tooltip.verified-user`) e badges ODS (`tooltip.ods-04/11/12/13`); expondo no contrato público `applyTooltips(root)` (idempotente, escopável, fallback pt-BR); os `22` HTMLs canônicos declaram `55` marcações `data-i18n-tooltip="tooltip.<nome>"` em toda tag com `title` estático não-vazio, sem conflito com `data-i18n-title` (usado apenas no `<html>` para page-title); `applyRuntimeI18n()` executa os cinco helpers no `DOMContentLoaded`; `scripts/hygiene-check.js` valida o gate declarativo da fase 3 via `runI18nTooltipChecks()`; nova suíte `tests/i18n-tooltip.test.js` (18 testes) e ajustes em `tests/kc-i18n.test.js` e `tests/i18n-metadata.test.js`; baseline expandida para **124/124 suites · 2546/2546 testes** | — |
...
- **Camada A — Continuação tática v11** (splits IIFE dos hotspots remanescentes):
  ...
  - `v12.5.0`–`v12.5.5`: `assets/js/controllers/profile.controller.js` (1463L → <700L) em `window._KCPR.*`
