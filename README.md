...
- status operacional do deploy em `09 de abril de 2026`: a branch `codex/v11-10-0-product-session-swr` foi publicada na PR `#240`, recebeu preview Vercel `READY` no deployment `dpl_FT1si5ByRtyzoQrNU1yEGqjHqYTJ`, foi mergeada na base `kinocampus-V11.0-foundations` no commit `7f4c1ac` e recebeu deploy manual de produção via `vercel deploy --prod --yes`, publicado em `https://kino-campus-70d1s6o4x-yannakamurabrs-projects.vercel.app` e aliasado em [www.kinocampus.com.br](https://www.kinocampus.com.br). As checagens locais confirmaram `assets/js/kc-api.client.js`, `assets/js/controllers/product.controller.js` e `assets/js/kc-comments.js` sintaticamente válidos, além das suites `tests/kc-api-client.test.js`, `tests/kc-api-session-swr.test.js`, `tests/kc-comments-session.test.js`, `tests/voting.test.js` e `tests/voting-session-hydration.test.js` verdes. No domínio publicado, `_product.html?id=1`, `assets/js/kc-api.client.js` e `assets/js/kc-comments.js` já refletem a hidratação de sessão, a invalidação pós-mutações e a revalidação silenciosa da iteração.
