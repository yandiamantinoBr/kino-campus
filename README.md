...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.5.1` concluída com o split `window._KCPR.presentation`: criado `assets/js/controllers/profile.presentation.js` (`518L`, `20 846` bytes, `28` exports), `assets/js/controllers/profile.controller.js` reduzido de **`1463L`** / `56 497` bytes para **`1261L`** / `48 514` bytes com `buildPresentationDeps()` + wrappers finos, `profile.html` sincronizado com a nova ordem de scripts e nova suíte `tests/profile.presentation.test.js` (14 testes); baseline expandida para **117/117 suites · 2442/2442 testes**; proxima iteracao: `v12.5.2` (split `window._KCPR.collections`).
...
| Testes | Jest: 117 suites de regressão e contrato |
...
- iteração encerrada: `v12.5.1` — **split `window._KCPR.presentation` concluido** (`profile.presentation.js` com `518L`, `20 846` bytes e `28` exports; `profile.controller.js` reduzido para `1261L` / `48 514` bytes)
...
- regressão: `117/117` suites, `2442/2442` testes verdes, hygiene `8.6.0`
...
- `assets/js/controllers/profile.controller.js`: apos `v12.5.1`, o core caiu de `1463L` / `56 497` bytes para `1261L` / `48 514` bytes; `assets/js/controllers/profile.presentation.js` passou a concentrar `28` exports em `518L` / `20 846` bytes, abrindo o primeiro boundary `_KCPR.*` sem alterar `window.KCProfileRefresh`
- próxima iteração: `v12.5.2` — split `window._KCPR.collections`
