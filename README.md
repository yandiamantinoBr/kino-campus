...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.4.5` concluída com o quinto split funcional do adapter local: criado `assets/js/adapters/local.posts-write.adapter.js` com namespace `window._KCLA.postsWrite` (**300L**, **7 exports**) para mutações de posts/drafts (`createPost`, `updatePost`, `deletePost`, `reportPost`, `togglePostStatus`, `renewPost`, `bumpPost`); `assets/js/adapters/local.adapter.js` caiu de `1119L` para **`1031L`** (`-88L`, `41 585` bytes) via wrappers finos, `getLocalPostsWriteModule()` e `buildLocalPostsWriteDeps()`; `22` HTMLs e `7` bootstraps de teste diretos atualizados; nova suíte `tests/local-posts-write.adapter.test.js` (**24 testes**); baseline expandida para **114/114 suites · 2383/2383 testes**; próxima iteração: `v12.4.6` (split `window._KCLA.profile`).
...
| Testes | Jest: 114 suites de regressão e contrato |
...
- iteração encerrada: `v12.4.5` — **split `window._KCLA.postsWrite` concluído** (`local.posts-write.adapter.js` com `300L`, `7` exports; `local.adapter.js` em `1031L` / `41 585` bytes; `22` HTMLs + `7` bootstraps diretos atualizados)
...
- regressão: `114/114` suites, `2383/2383` testes verdes, hygiene `8.6.0`
...
- sub-adapters `window._KCLA.*` operacionais: `notifications`, `ratings`, `saved`, `postsRead`, `postsWrite` (5 total)
...
- `local.adapter.js`: após `v12.4.5`, o core caiu de `1862L` / `75 712` bytes para `1031L` / `41 585` bytes; `assets/js/adapters/local.notifications.adapter.js` segue com `250L`, `14` exports, `assets/js/adapters/local.ratings.adapter.js` com `339L`, `6` exports, `assets/js/adapters/local.saved.adapter.js` com `252L`, `7` exports, `assets/js/adapters/local.posts-read.adapter.js` com `687L`, `8` exports, e `assets/js/adapters/local.posts-write.adapter.js` entrou com `300L`, `7` exports; os `22` HTMLs consumidores e `7` bootstraps diretos de teste já carregam a cadeia `notifications -> ratings -> saved -> posts-read -> posts-write -> local.adapter.js`
- próxima iteração: `v12.4.6` — split `window._KCLA.profile`
