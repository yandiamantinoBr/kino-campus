...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.8.0` concluída com auditoria estrutural de acessibilidade (docs-only): `docs/a11y-audit-v12.8.md` mapeia `7` problemas WCAG 2.1 AA nos `22` HTMLs (`h1` ausente em `10` páginas, skip link ausente em `21`, `<nav>` sem `aria-label` em `17`, `3` selects admin sem label, `2` botões icon-only sem `aria-label`, `1` label sem `for`) e define o plano de correção completo para `v12.8.1`; baseline preservada em **125/125 suites · 2562/2562 testes**; próxima iteração: `v12.8.1` (correções a11y estruturais + `tests/a11y.test.js`).
