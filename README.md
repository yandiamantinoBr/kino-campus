...
**Status atual:** v11 concluída (v11.1.0–v11.33.7); v12 em execução (*Consolidação & Qualidade Sistêmica*) — `v12.7.3` concluída com gate formal da trilha B2 i18n: `scripts/hygiene-check.js` ganhou `runI18nB2GateChecks()` validando `>= 440` chaves e `>= 800` linhas em `kc-i18n.js` e totais de markings (`>= 189` aria-label, `>= 59` placeholder, `>= 55` tooltip, `>= 5` alt); criados `tests/i18n-b2-gate.test.js` (16 testes de regressão em 3 grupos) e `docs/i18n-b2-coverage-v12.7.md` (auditoria completa da trilha B2 com 440 chaves, 352 marcações, 5 superfícies); a trilha B2 está formalmente encerrada; baseline expandida para **125/125 suites · 2562/2562 testes**; próxima iteração: `v12.8.x` (trilha B3 — a11y audit estrutural).
