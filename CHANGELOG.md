...
- `v12.2.6`: `assets/js/kc-utils.js` reduzido de `1168L` -> `531L` (`-637L`): 9 corpos do dominio presentation substituidos por delegation wrappers (`cssEscape`, `inferCaronasRoute`, `inferAchadosLocation`, `inferOportunidadesSubcategory`, `inferEventosCategory`, `applyPresentationRules`, `getDisplayMarkerTags`, `renderMarkerTags`, `renderPostCard`). Acumulado desde o monolito original: `2445L` -> `531L` (`-1914L` em 7 splits); gate estrutural `<900L` atingido.
