...
| `v11.20.1` | persistir preferências de notificação por evento e canal | `settings.html`, `KCAPI`, adapters, migration de preferências | em fechamento operacional na PR `#271`, com camada privada `notification_preferences`, UI em `settings` e triggers atuais respeitando `in_app` |
...
- PR / commit / deploy:
  PR funcional `#271`, commit funcional `0fcef67` na branch `codex/v11-20-1-notification-preferences`, preview `dpl_HrWK6p9ugp8LZ9PSfKgLbJ4m8Q7U`. Os metadados finais de merge squash e deploy de produção devem ser preenchidos no fechamento operacional desta mesma iteração.
- riscos residuais:
