...
- validacao em navegador:
  `www.kinocampus.com.br` respondeu `200 OK` com `kc-i18n.js` confirmado no HTML em `dpl_9Pm65XqZSx26BWRNAkWu59zR8A1C`, todos confirmados em `12 de abril de 2026`.
- PR \ commit \ deploy:
...
- validacao operacional:
  docs-only; nenhum comportamento de produção alterado.
- validacao em navegador:
  `www.kinocampus.com.br` respondeu `200 OK` com `kc-i18n.js` confirmado no HTML em `dpl_Dje9wnUCjvJCZtsALDLf2gjDtZvH`, todos confirmados em `12 de abril de 2026`.
- PR \ commit \ deploy:
  PR `#292`, commit funcional `19c09f1` na branch `codex/v11-25-1-changelog-consolidation`, merge squash `ef2f819` na base `kinocampus-V11.0-foundations`, preview `dpl_ADy67WeTEq5v4ZfGE7X91z7PkKfz`, deployment pós-merge `dpl_2Z3eLr4hb1BeuPMQ8KesFt3rarnf`, produção `dpl_Dje9wnUCjvJCZtsALDLf2gjDtZvH` (promote from `dpl_2Z3eLr4hb1BeuPMQ8KesFt3rarnf`), todos confirmados em `12 de abril de 2026`.
- próximas fases:
...
|---|---|
| Data | 15 de abril de 2026 |
| Branch | `codex/v11-30-1-adapter-split-notifications-analytics` |
| Tipo | refactor (split de monolito, sem alteração de comportamento) |
| PR | `#321` |

- objetivo:
  extrair os 2 grupos de menor acoplamento do `supabase.adapter.js` (4041L → 3626L, −415L) para sub-adapters independentes usando o namespace `window._KCSA`.
- resultado:
  - **NOVO** `assets/js/adapters/supabase.analytics.adapter.js` — 6 funções de analytics (`getTopContributors`, `trackCouponClick`, `trackShare`, `trackView`, `getPostAnalytics`, `checkDuplicatePost`) registradas em `window._KCSA.analytics`.
  - **NOVO** `assets/js/adapters/supabase.notifications.adapter.js` — 11 funções + 6 helpers de normalização (`preferences`, `channel targets`, `phone formatting`) registrados em `window._KCSA.notifications`.
  - **ALTERADO** `supabase.adapter.js`: expõe `window._KCSA.getClient` e `getCurrentUser`; `driverSupabase` aponta para `window._KCSA.analytics.*` e `notifications.*`; fallback guard para graceful degradation em testes.
  - **22 HTMLs** atualizados: 2 novas tags `<script defer>` inseridas antes de `supabase.adapter.js` (path relativo correto por public vs. admin).
- arquivos alterados:
  - `assets/js/adapters/supabase.analytics.adapter.js` — novo sub-adapter.
  - `assets/js/adapters/supabase.notifications.adapter.js` — novo sub-adapter.
  - `assets/js/adapters/supabase.adapter.js` — refatorado (−415L).
  - 22 HTMLs públicos e admin — 2 script tags adicionadas.
  - `tests/supabase-analytics-adapter.test.js` — 14 testes estáticos.
  - `tests/supabase-notifications-adapter.test.js` — 20 testes estáticos.
- resultado dos testes:
  `63/63` suites, `778/778` testes; hygiene `8.6.0`.
- PR \ commit \ deploy:
...
- validacao operacional:
  testes-only; zero alteração em `assets/js/kc-create-post.js`, zero mudança de HTML carregador, zero impacto em Supabase ou Edge Functions.
- validacao em navegador:
  o preview da branch ficou `READY` em `dpl_4EA41nrFzQ6STZpkXbG7FvsmoJEN`; o deployment pós-merge da base ficou `READY` em `dpl_CRC3gdHL4gwwGCTenk1MpRcXPLeZ`; a produção promovida em [www.kinocampus.com.br](https://www.kinocampus.com.br) respondeu `200` no deployment `dpl_ACgiQPzePCo3RrRsZATuxvYocu1o`.
- PR \ commit \ deploy:
  PR `#359` — squash merge `63b4e12` (commits da branch `5ab55ad` e `fbca924`) — preview `dpl_4EA41nrFzQ6STZpkXbG7FvsmoJEN` — pós-merge `dpl_CRC3gdHL4gwwGCTenk1MpRcXPLeZ` — produção `dpl_ACgiQPzePCo3RrRsZATuxvYocu1o` (promote from `dpl_CRC3gdHL4gwwGCTenk1MpRcXPLeZ`), smoke HTTP 200 com `https://www.kinocampus.com.br/?ts=1776429840`.
- próximas fases:
...
| Campo | Valor |
|-------|-------|
| Branch | `codex/v11-31-4-kc-create-post-resolvers-split` |
| Base | `kinocampus-V11.0-foundations` |
| Tipo | extração estrutural — resolvers e normalizadores de domínio |
| Escopo | `assets/js/kc-create-post.resolvers.js` (NOVO), `assets/js/kc-create-post.js`, `12` HTMLs carregadores, `tests/kc-create-post-resolvers.test.js` (NOVO), `README.md`, `RELATORIO-KINOCAMPUS-V11.md` |

- objetivo:
  extrair do hotspot `kc-create-post.js` os 25 resolvers e normalizadores de domínio (oportunidades: tipo, área, modalidade, regime; moradia: tipo, região, marcadores, sync DOM; caronas: local, opções, sync DOM; achados/perdidos: local, opções, sync DOM; utilitários de array), registrando a lógica em um IIFE dedicado `kc-create-post.resolvers.js` via `window._KCCreatePost.resolvers`, com stubs de delegação no core e fallbacks seguros para cada função.
- resultado:
  - **NOVO** `assets/js/kc-create-post.resolvers.js` — IIFE `'use strict'` com `window._KCCreatePost.resolvers`, contendo: `_esc()`, `_getState()` e 25 funções organizadas em seções por domínio (oportunidades, moradia, caronas, achados/perdidos); namespace exporta todos os 25 membros.
  - **ALTERADO** `assets/js/kc-create-post.js` — bloco de resolvers substituído por `_kcResolversModule()` + 25 stubs de delegação com fallbacks seguros (arrays vazios, objetos neutros, strings vazias); `kc-create-post.js` reduz de `2342L` para `2113L` (−229L).
  - **ALTERADOS** `12` HTMLs da base principal — agora carregam `assets/js/kc-create-post.resolvers.js` com `defer` após `assets/js/kc-create-post.media.js`.
  - **NOVO** `tests/kc-create-post-resolvers.test.js` — suíte estática com `81` testes cobrindo: estrutura IIFE/namespace (6), utilitários locais (2), normalizeOpportunityTypeKey (4), getOpportunityTypeOptionKey (3), resolveOpportunityAreaValue (4), getOpportunityAreaOptions (3), resolveOpportunityWorkMode (3), resolveOpportunityRegime (3), normalizeHousingTypeKey (3), getHousingTypeOptionKey (3), parseStringArray (4), serializeHousingFeatureValues (3), resolveHousingRegionValue (4), getHousingRegionOptions (3), resolveHousingFeatureValues (4), getHousingFeatureOptions (3), syncHousingFeatureField (5), appendHousingFeatureFromInput (3), caronas (6), syncHousingRegionInput (4), achados/perdidos (7), membros do namespace (1).
- achados principais:
  - as 3 funções de sync DOM que escrevem em `kcCreateState.values` (`kcSyncHousingRegionInput`, `kcSyncHousingFeatureField`, `kcSyncLostFoundLocationInput`) usam `_getState()` → `window._KCCreatePost._state`, seguindo o mesmo padrão do módulo `media`.
  - todos os 25 stubs têm fallbacks seguros: arrays vazios para listas de opções, objetos `{ key: '', label: '' }` para resolvers de domínio, `null` para sync DOM — o core não lança exceções quando o módulo não está disponível.
  - `kcParseStringArrayValue` também foi extraído para o módulo (apesar de ser usado em `kcCaptureCreateValues` e `kcReadActiveCreateArrayValue` no core) — o stub no core delega lazily, seguro porque esses pontos de chamada só executam após `DOMContentLoaded`.
- arquivos alterados:
  - `assets/js/kc-create-post.resolvers.js` (NOVO)
  - `assets/js/kc-create-post.js` (ALTERADO, `2342L -> 2113L`, −229L)
  - `_product.html` (ALTERADO)
  - `achados-perdidos.html` (ALTERADO)
  - `caronas-feed.html` (ALTERADO)
  - `compra-venda-feed.html` (ALTERADO)
  ...
  - `oportunidades.html` (ALTERADO)
  - `search-results.html` (ALTERADO)
  - `tests/kc-create-post-resolvers.test.js` (NOVO, `487L`)
  - `README.md` (ALTERADO)
  - `RELATORIO-KINOCAMPUS-V11.md` (ALTERADO)
- resultado dos testes:
  baseline elevada para `84/84` suites e `1459/1459` testes; hygiene `8.6.0`.
- validacao operacional:
  `node --check assets/js/kc-create-post.js`, `node --check assets/js/kc-create-post.resolvers.js`, `npx jest tests/kc-create-post-resolvers.test.js --runInBand`, `npx jest --passWithNoTests --runInBand`, `node scripts/hygiene-check.js`.
- validacao em navegador:
  deployment `dpl_HPWxVQNXF2c6yEdySMuEqsWsipw4` promovido para produção; `www.kinocampus.com.br` respondeu `200` com smoke `https://www.kinocampus.com.br/?ts=1745889600`.
- PR \ commit \ deploy:
...
| Campo | Valor |
|-------|-------|
| Branch | `codex/v11-31-6-kc-create-post-submit-split` |
| Base | `kinocampus-V11.0-foundations` |
| Tipo | extração estrutural — pipeline de submit e edição |
| Escopo | `assets/js/kc-create-post.submit.js` (NOVO), `assets/js/kc-create-post.js`, `12` HTMLs carregadores, `tests/kc-create-post-submit.test.js` (NOVO), `tests/kc-create-post-contract.test.js` (ALTERADO), `tests/kc-create-post-active-fields.test.js` (ALTERADO), `README.md`, `RELATORIO-KINOCAMPUS-V11.md` |

- objetivo:
  extrair do hotspot `kc-create-post.js` a função `kcHandleCreateSubmit` — o pipeline completo de submit e edição de publicações (validação de formulário, resolução de campos ativos, construção do payload, modo edição via `KCAPI.updatePost`, modo criação Supabase vs local, verificação de duplicatas, audit log, upsert de locais custom de caronas e redirect pós-publicação) — registrando-a em um IIFE dedicado `kc-create-post.submit.js` via `window._KCCreatePost.submit`, com stub de delegação async no core e fallback `undefined` seguro.
- resultado:
  - **NOVO** `assets/js/kc-create-post.submit.js` — IIFE `'use strict'` com `window._KCCreatePost.submit`, contendo: `_getState()` (acesso ao estado) e `handleCreateSubmit()` async (707L extraídas); acessa `kcCreateState` via `_getState()`; todos os demais globais (`KCAPI`, `showToast`, `kcModulePage`, `kcResolve*`, `kcNormalize*`, `kcGetActiveCreateFieldNames`, etc.) acessados diretamente do scope do browser — carregados antes em `kc-core.js` e `kc-create-post.js`; namespace exporta `handleCreateSubmit`.
  - **ALTERADO** `assets/js/kc-create-post.js` — `kcHandleCreateSubmit` substituído por `_kcSubmitModule()` + stub de delegação lazy async; core reduz de `1959L` para `1260L` (−699L).
  - **ALTERADOS** `12` HTMLs da base principal — agora carregam `assets/js/kc-create-post.submit.js` com `defer` após `assets/js/kc-create-post.fields.js`. Ordem canônica: `schema → core → media → resolvers → fields → submit`.
  - **NOVO** `tests/kc-create-post-submit.test.js` — suíte estática com `68` testes cobrindo: estrutura IIFE/namespace (5), `_getState()` (3), `handleCreateSubmit` presença/assinatura (4), captura e validação (7), campos ativos e payload (10), resolvers de domínio (10), modo edição (4), modo criação supabase vs local (8), audit log e redirect (6), tratamento de erros/finally (3), namespace (2), stubs no core (5).
  - **ALTERADO** `tests/kc-create-post-contract.test.js` — testes de submit pipeline migrados para verificar `submitSource` (v11.31.6); `submitSource` carregado em `beforeAll`.
  - **ALTERADO** `tests/kc-create-post-active-fields.test.js` — teste 'submit path derives active field names' atualizado para ler `kc-create-post.submit.js`.
- achados principais:
  - `kcHandleCreateSubmit` é a função mais longa do arquivo (707L, ~33KB de fonte) — a extração sozinha reduziu o core em 36% da linha count atual.
  - Todos os globals que `handleCreateSubmit` usa (`KCAPI`, `KCAPI.updatePost`, `showToast`, `kcModulePage`, etc.) são window globals já carregados antes do sub-módulo — sem necessidade de lazy accessors adicionais.
  - `kcCreateState` é passado por referência via `_getState()` → `window._KCCreatePost._state`; mutações no sub-módulo (`kcCreateState.submitting = true`, `kcCreateState.values[name] = ...`) propagam corretamente ao estado compartilhado.
  - 5 falhas nos testes existentes corrigidas: `kc-create-post-contract.test.js` (4 testes verificavam strings do submit no core; migrados para `submitSource`) e `kc-create-post-active-fields.test.js` (1 teste verificava `const activeFieldNames = kcGetActiveCreateFieldNames(` no core; atualizado para `submitSource`).
- arquivos alterados:
  - `assets/js/kc-create-post.submit.js` (NOVO, ~540L)
  - `assets/js/kc-create-post.js` (ALTERADO, `1959L → 1260L`, −699L)
  - `_product.html` (ALTERADO)
  - `achados-perdidos.html` (ALTERADO)
  - `caronas-feed.html` (ALTERADO)
  - `compra-venda-feed.html` (ALTERADO)
  ...
  - `oportunidades.html` (ALTERADO)
  - `search-results.html` (ALTERADO)
  - `tests/kc-create-post-submit.test.js` (NOVO, 374L, 68 testes)
  - `tests/kc-create-post-contract.test.js` (ALTERADO, +10L)
  - `tests/kc-create-post-active-fields.test.js` (ALTERADO, +4L)
  - `README.md` (ALTERADO)
  - `RELATORIO-KINOCAMPUS-V11.md` (ALTERADO)
- resultado dos testes:
  baseline elevada para `86/86` suites e `1598/1598` testes.
- validacao operacional:
  `node --check assets/js/kc-create-post.submit.js`, `node --check assets/js/kc-create-post.js`, `npx jest tests/kc-create-post-submit.test.js --no-coverage`, `npx jest --no-coverage` (86/86, 1598/1598).
- validacao em navegador:
  deployment `dpl_EDPrZXfYYJq4c6J3Qvga2oYHYnwd` promovido para produção via `npx vercel promote`; `www.kinocampus.com.br` respondeu `200`.
- PR \ commit \ deploy:
  PR `#368` — fast-forward merge `bba891e` — produção `dpl_EDPrZXfYYJq4c6J3Qvga2oYHYnwd` (promote via `npx vercel promote`), smoke HTTP 200.
- próximas fases:
  trilha `v11.31.x` encerrada. Core estável em `648L`.

### Iteração `v11.31.7`

| Campo | Valor |
|-------|-------|
| Branch | `codex/v11-31-7-kc-create-post-render-split` |
| Base | `kinocampus-V11.0-foundations` |
| Tipo | extração estrutural — render/modal (última da trilha v11.31.x) |
| Escopo | `assets/js/kc-create-post.render.js` (NOVO), `assets/js/kc-create-post.js`, `12` HTMLs carregadores, `tests/kc-create-post-render.test.js` (NOVO), `tests/kc-create-post-contract.test.js` (ALTERADO), `README.md`, `RELATORIO-KINOCAMPUS-V11.md` |

- objetivo:
  extrair do hotspot `kc-create-post.js` as 6 funções de render e bootstrap do modal — `_kcFormatDescriptionField`, `_kcUpdateDescPreview`, `kcCreateSustainSectionHtml`, `kcCreateVisibilitySectionHtml`, `kcEnsureCreateModal` e `kcRenderCreateModal` — para um IIFE dedicado `kc-create-post.render.js` via `window._KCCreatePost.render`, encerrando a trilha `v11.31.x` com o core abaixo de 700L.
- resultado:
  - **NOVO** `assets/js/kc-create-post.render.js` — IIFE `'use strict'` com `window._KCCreatePost.render`; helpers privados: `_getState()`, `_getModalId()`, `_getModules()`, `_getVisibilityOptions()`, `_esc()`; todas as referências a `KC_CREATE_MODAL_ID`, `KC_CREATE_SCHEMA` e `KC_POST_VISIBILITY_OPTIONS` substituídas pelos lazy accessors; `kcCreateState` acessado via `_getState()` em cada handler de evento (sem captura de referência no setup do modal); `kcRenderCreateModal` declara `const kcCreateState = _getState()` e `const KC_CREATE_SCHEMA = _getModules()` localmente.
  - **ALTERADO** `assets/js/kc-create-post.js` — 6 corpos de função substituídos por `_kcRenderModule()` + stubs de delegação; `_esc` removido (agora interno ao render module); core reduz de `1260L` para `648L` (meta <700L ✓, −612L).
  - **ALTERADOS** `12` HTMLs da base principal — agora carregam `assets/js/kc-create-post.render.js` com `defer` após `assets/js/kc-create-post.submit.js`. Ordem canônica: `schema → core → media → resolvers → fields → submit → render`.
  - **NOVO** `tests/kc-create-post-render.test.js` — suíte estática com `87` testes cobrindo: estrutura IIFE (5), helpers privados (5), `_kcFormatDescriptionField` (8), `_kcUpdateDescPreview` (4), `kcCreateSustainSectionHtml` (4), `kcCreateVisibilitySectionHtml` (5), `kcEnsureCreateModal` estrutura (6), `kcEnsureCreateModal` handlers (8), `kcRenderCreateModal` (14), namespace (7), stubs no core (8).
  - **ALTERADO** `tests/kc-create-post-contract.test.js` — RENDER_SRC e `renderSource` adicionados em `beforeAll`; testes de modal bootstrap e render pipeline migrados para verificar `renderSource`; stubs no core verificados em `source`.
- achados principais:
  - `kcEnsureCreateModal` (~273L) e `kcRenderCreateModal` (~263L) somavam ~536L das ~638L extraídas; a maior complexidade estava nos event handlers de `kcEnsureCreateModal` que acessam estado mutável — solucionado com `const st = _getState()` no início de cada closure.
  - `KC_CREATE_MODAL_ID`, `KC_CREATE_SCHEMA` e `KC_POST_VISIBILITY_OPTIONS` permanecem como `const` no core (contrato de testes exige suas declarações); o render module lê os mesmos valores via lazy accessors sobre `window._KCCreatePost.schema.*`.
  - Nenhum teste existente quebrou além dos que verificavam as strings extraídas em `source` (4 no contract test) — todos migrados para `renderSource`.
- arquivos alterados:
  - `assets/js/kc-create-post.render.js` (NOVO, ~730L)
  - `assets/js/kc-create-post.js` (ALTERADO, `1260L → 648L`, −612L)
  - `_product.html`, `achados-perdidos.html`, `caronas-feed.html`, `compra-venda-feed.html`, `create-post.html`, `eventos.html`, `index.html`, `moradia.html`, `my-posts.html`, `ods.html`, `oportunidades.html`, `search-results.html` (ALTERADOS, 12 arquivos)
  - `tests/kc-create-post-render.test.js` (NOVO, ~440L, 87 testes)
  - `tests/kc-create-post-contract.test.js` (ALTERADO)
  - `README.md` (ALTERADO)
  - `RELATORIO-KINOCAMPUS-V11.md` (ALTERADO)
- resultado dos testes:
  baseline elevada para `87/87` suites e `1672/1672` testes.
- validacao operacional:
  `node --check assets/js/kc-create-post.render.js`, `node --check assets/js/kc-create-post.js`, `npx jest tests/kc-create-post-render.test.js --no-coverage`, `npx jest --no-coverage` (87/87, 1672/1672).
- PR \ commit \ deploy:
...
| Campo | Valor |
|-------|-------|
| Branch | `codex/v11-32-1-kcapi-contract-freeze` |
| Base | `kinocampus-V11.0-foundations` |
| Tipo | hardening de QA/contrato |
| Escopo | `tests/kc-api-facade-contract.test.js` (NOVO), `README.md`, `RELATORIO-KINOCAMPUS-V11.md` |

- objetivo:
  congelar o contrato publico de `window.KCAPI` em uma suite estatica dedicada antes do primeiro split da trilha `v11.32.x`, travando shape da fachada, exports por dominio, fallbacks entre drivers, guards de indisponibilidade e os hooks mais sensiveis de cache/SWR sem alterar runtime.
- resultado:
  - **NOVO** `tests/kc-api-facade-contract.test.js` — suite estatica dedicada ao facade `window.KCAPI`, com 16 testes cobrindo IIFE/`'use strict'`, `Object.freeze`, registry de adapters, getter `activeDriver`, exports publicos por dominio, guards de indisponibilidade, fallbacks de local/Supabase, contratos de preferencias/targets de notificacao, hooks de cache/SWR de analytics/comentarios e globals de diagnostico.
  - **ALTERADO** `README.md` — status da base movido para `v11.32.1`, baseline atualizada para `88/88` suites e `1688/1688` testes, nova entrega recente registrada e progresso atual apontando o primeiro split de `notifications`.
  - **ALTERADO** `RELATORIO-KINOCAMPUS-V11.md` — estado macro da fase atualizado e registro completo desta iteracao adicionado.
- achados principais:
  - a fachada publica `window.KCAPI` agora tem uma suite propria para travar o contrato transversal sem depender de execucao do runtime nem de alterar `assets/js/kc-api.client.js`.
  - o primeiro split seguro continua sendo `notifications`, porque o contrato de registry, fallbacks, cache/SWR e dominios publicos ja ficou blindado em nivel de source-contract.
  - nenhuma alteracao de runtime foi necessaria nesta fase; a iteracao e inteiramente de QA/contrato.
- resultado dos testes:
  baseline elevada para `88/88` suites e `1688/1688` testes; hygiene alvo segue `8.6.0`.
- validacao operacional:
  `npx jest --passWithNoTests --runInBand`, `node scripts/hygiene-check.js` e `git diff --check`.
- validacao em navegador:
  preview pos-merge `dpl_Fpe97zRfS3kXaJrixXW2SPtSrqVQ` ficou `READY`; a base foi promovida para producao em `dpl_77TBebo7PaY6iVmzuvwekUxqW79e`, e o smoke HTTP confirmou `200` em `https://www.kinocampus.com.br/?ts=1776465042`, `https://www.kinocampus.com.br/compra-venda-feed.html?ts=1776465043`, `https://www.kinocampus.com.br/create-post.html?ts=1776465044` e `https://www.kinocampus.com.br/_product.html?id=1&ts=1776465045`.
- PR \ commit \ deploy:
  PR `#372` — squash merge `e992012` — preview pos-merge `dpl_Fpe97zRfS3kXaJrixXW2SPtSrqVQ` — producao `dpl_77TBebo7PaY6iVmzuvwekUxqW79e` (promote via `vercel promote`), smoke HTTP `200`.
- próximas fases:
...
| Campo | Valor |
|-------|-------|
| Branch | `codex/v11-32-2-kcapi-notifications-split` |
| Base | `kinocampus-V11.0-foundations` |
| Tipo | split estrutural + hardening de contrato/publicacao |
| Escopo | `assets/js/kc-api.notifications.js` (NOVO), `assets/js/kc-api.client.js`, `tests/kc-api-notifications-module.test.js` (NOVO), `tests/kc-api-facade-contract.test.js`, `tests/anti-spam.test.js`, `tests/kc-api-client.test.js`, `tests/kc-api-notification-preferences-contract.test.js`, `tests/kc-api-notifications-contract.test.js`, `tests/kc-api-session-swr.test.js`, `tests/post-analytics.test.js`, `22` HTMLs carregadores, `README.md`, `RELATORIO-KINOCAMPUS-V11.md` |

- objetivo:
  executar o primeiro split seguro por dominio da trilha `v11.32.x`, extraindo `notifications` da fachada `window.KCAPI` para um submodulo proprio com namespace interno `window._KCAPI.notifications`, sem alterar assinatura publica, fallback entre drivers, cache/SWR nem comportamento visivel da interface.
- resultado:
  - **NOVO** `assets/js/kc-api.notifications.js` — IIFE com `'use strict'`, namespace `window._KCAPI.notifications` e `11` exports do dominio: fallbacks de preferencias/targets privados, feed in-app, mark-read, clear, unread count e subscribe/unsubscribe.
  - **ALTERADO** `assets/js/kc-api.client.js` — o facade publico `window.KCAPI` passou a delegar todo o dominio `notifications` via `getNotificationsModule()` com guard defensivo, preservando fallbacks e contratos publicos existentes.
  - **NOVO** `tests/kc-api-notifications-module.test.js` — suite estatica dedicada ao novo submodulo, cobrindo IIFE/namespace, exports, delegacao de driver, fallbacks e a ordem de carregamento do asset em `22` HTMLs.
  - **ALTERADO** `tests/kc-api-facade-contract.test.js` — contrato do facade ampliado para travar `window._KCAPI.notifications`, `getNotificationsModule()` e a delegacao do dominio extraido.
  - **ALTERADO** `tests/anti-spam.test.js`, `tests/kc-api-client.test.js`, `tests/kc-api-notification-preferences-contract.test.js`, `tests/kc-api-notifications-contract.test.js`, `tests/kc-api-session-swr.test.js` e `tests/post-analytics.test.js` — bootstrap dos testes runtime atualizado para carregar `assets/js/kc-api.notifications.js` antes do facade.
  - **ALTERADOS** `account-setup.html`, `achados-perdidos.html`, `ajuda.html`, `auth-callback.html`, `caronas-feed.html`, `compra-venda-feed.html`, `create-post.html`, `eventos.html`, `index.html`, `moradia.html`, `my-posts.html`, `ods.html`, `oportunidades.html`, `profile.html`, `search-results.html`, `settings.html`, `_product.html`, `admin/banners.html`, `admin/help-requests.html`, `admin/index.html`, `admin/moderation.html` e `admin/reports.html` — todos passam a carregar `assets/js/kc-api.notifications.js` antes de `assets/js/kc-api.client.js`.
  - **ALTERADO** `README.md` — status da base movido para `v11.32.2`, baseline atualizada para `89/89` suites e `1694/1694` testes, nova entrega recente registrada e progresso atual apontando `saved/highlights` como proxima fatia.
  - **ALTERADO** `RELATORIO-KINOCAMPUS-V11.md` — estado macro da fase atualizado e registro completo desta iteracao adicionado.
- achados principais:
  - o primeiro split da trilha `KCAPI` foi fechado sem quebrar a interface publica `window.KCAPI`; a separacao ficou totalmente interna via `window._KCAPI.notifications`.
  - o dominio `notifications` mostrou acoplamento baixo o suficiente para ser extraido primeiro, mantendo `local.adapter.js` e `supabase.adapter.js` como superficies equivalentes intactas.
  - o facade principal continuou com footprint alto (`2541L`) porque ainda concentra delegacao e contrato transversal; os ganhos de reducao de linhas devem aparecer mais claramente nas proximas extracoes em sequencia.
  - o gate operacional da trilha permaneceu Jest + hygiene + `vercel inspect` + smoke HTTP `200`; previews continuam SSO-protected no fetch direto, entao a evidencia canonica do preview foi `vercel inspect` + checks verdes da PR.
- resultado dos testes:
  baseline elevada para `89/89` suites e `1694/1694` testes; hygiene alvo segue `8.6.0`.
- validacao operacional:
  `npx jest --passWithNoTests --runInBand`, `node scripts/hygiene-check.js` e `git diff --check`.
- validacao em navegador:
  preview pos-merge `dpl_Dqf1SeP5LDRsHrsmm7NKvT1u2cA5` ficou `READY`; a base foi promovida para producao em `dpl_A7RE828Wab7S2W6UG2vzpUyr8Ljo`, e o smoke HTTP confirmou `200` em `https://www.kinocampus.com.br/?ts=1776466901`, `https://www.kinocampus.com.br/compra-venda-feed.html?ts=1776466902`, `https://www.kinocampus.com.br/create-post.html?ts=1776466903` e `https://www.kinocampus.com.br/_product.html?id=1&ts=1776466904`. O HTML publico de `create-post.html` e `_product.html` tambem foi verificado com `curl.exe --ssl-no-revoke -L`, confirmando `assets/js/kc-api.notifications.js` antes de `assets/js/kc-api.client.js`.
- PR \ commit \ deploy:
  PR `#374` — squash merge `22825eb` — preview pos-merge `dpl_Dqf1SeP5LDRsHrsmm7NKvT1u2cA5` — producao `dpl_A7RE828Wab7S2W6UG2vzpUyr8Ljo` (promote via `vercel promote`), smoke HTTP `200`.
- próximas fases:
  `v11.32.3` (split `saved/highlights`) -> `v11.32.4` (split `help/invites`) -> `v11.32.5` (split `posts-read`) -> `v11.32.6` (comments/votes + hardening) -> `v11.32.7` (release gate).
