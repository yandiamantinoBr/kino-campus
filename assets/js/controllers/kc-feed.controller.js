...
(function () {
  ...
  // V8.1.4.2: padroniza paginação no read path (paridade local/supabase)
  ...
  async function injectFeed(options) {
    ...
    const page = (opt.page != null) ? Math.max(1, parseInt(String(opt.page), 10) || DEFAULT_PAGE) : DEFAULT_PAGE;
    const limit = (opt.limit != null) ? Math.max(1, parseInt(String(opt.limit), 10) || POSTS_LIMIT) : POSTS_LIMIT;

    try {
      const mkLabel = moduleKeys.length ? moduleKeys.join(', ') : 'all';
      console.debug(`[KCFeed] Carregando página ${page} (limit ${limit}) módulos: ${mkLabel}`);
    } catch (_) {}
    ...
    try {
      ...
      if (moduleKeys.length === 0) {
        dbPosts = await window.KCAPI.getPosts({ page, limit });
      } else if (moduleKeys.length === 1) {
        dbPosts = await window.KCAPI.getPosts({ module: moduleKeys[0], page, limit });
      } else {
      ...
      // V8.1.4.2: garante limite real na UI (inclui user-posts)
      const limited = (Number.isFinite(limit) && limit > 0) ? normalized.slice(0, limit) : normalized;

      const html = limited.map((p) => window.KCUtils.renderPostCard(p, ctx)).join('');
      if (!html.trim()) return;
