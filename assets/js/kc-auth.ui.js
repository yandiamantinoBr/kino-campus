...
(function () {
  ...
  function isAllowedDomain(email, allowedDomains) {
    ...
    if (!d) return false;
    return list.some((ad) => d === String(ad).toLowerCase() || d.endsWith('.' + String(ad).toLowerCase()));
  }
  ...
  function refreshHeaderLabel(user) {
    ...
    if (user && user.email) {
      ...
      const allowlistActive = Array.isArray(allowedDomains) && allowedDomains.filter(Boolean).length > 0;
      const allowlisted = allowlistActive ? isAllowedDomain(user.email, allowedDomains) : false;
      const verified = !!(profile && (profile.verified === true)) || allowlisted;
