(function () {
  ...
  function togglePasswordVisibility(button) {
    const inputId = button.getAttribute('data-kc-password-toggle');
  ...
  function ensureModal() {
    ...
    modal.innerHTML = [
      ...
      '<section class="kc-auth-panel" data-auth-panel="login"><form class="kc-auth-form" id="kcAuthLoginForm"><div class="kc-auth-field"><label for="kcAuthLoginEmail">' + _t('auth.modal-email-institutional', 'E-mail institucional') + '</label><input id="kcAuthLoginEmail" name="email" type="email" autocomplete="email" placeholder="' + _t('auth.modal-email-placeholder', 'voce@ufg.br') + '" required /></div><div class="kc-auth-field kc-auth-field--password"><label for="kcAuthLoginPassword">' + _t('auth.modal-password-label', 'Senha') + '</label><div class="kc-auth-input-wrap"><input id="kcAuthLoginPassword" name="password" type="password" autocomplete="current-password" minlength="6" placeholder="' + _t('auth.modal-current-password', 'Sua senha') + '" required /><button type="button" class="kc-auth-password-toggle" data-kc-password-toggle="kcAuthLoginPassword" aria-label="Mostrar senha" aria-pressed="false"><i class="fas fa-eye" aria-hidden="true"></i></button></div></div><div class="kc-auth-actions"><button class="kc-auth-btn primary" type="submit">' + _t('auth.modal-enter-btn', 'Entrar') + '</button></div></form><div class="kc-auth-links kc-auth-links--split"><button type="button" class="kc-auth-link-btn kc-auth-link-btn--inline" data-auth-link="forgot">' + _t('auth.modal-forgot-link', 'Esqueci minha senha') + '</button><button type="button" class="kc-auth-link-btn kc-auth-link-btn--inline" data-auth-link="signup">' + _t('auth.modal-no-account-link', 'Ainda não tenho conta') + '</button><button type="button" class="kc-auth-link-btn kc-auth-link-btn--quiet kc-auth-link-btn--full" data-auth-link="resend">' + _t('auth.modal-resend-link', 'Reenviar confirmação') + '</button></div></section>',
  ...
  function bindModalEvents() {
    ...
    modal.addEventListener('click', function (event) {
      ...
      const passwordToggle = event.target.closest('[data-kc-password-toggle]');
      if (passwordToggle) {
