...
describe('mobile bottom nav', () => {
  test.each(PAGES_WITH_STATIC_MOBILE_NAV)('%s usa Oportunidades como item fixo', (page) => {
    ...
    expect(nav).toContain('href="/oportunidades"');
    expect(nav).toContain('fas fa-briefcase');
    expect(nav).toContain('Oportunidades');
    expect(nav).not.toContain('href="/compra-venda"');
  ...
  test('o estado ativo distingue Oportunidades de Compra e Venda', () => {
    ...
    expect(oportunidadesNav).toMatch(/<a\s+class="active"\s+href="\/oportunidades"|<a\s+href="\/oportunidades"\s+class="active"/);
    expect(compraVendaNav).toContain('kc-menu-toggle active');
    expect(compraVendaNav).not.toMatch(/<a\s+class="active"\s+href="\/oportunidades"|<a\s+href="\/oportunidades"\s+class="active"/);
  });
  ...
  test('kc-core e kc-public-shell usam o mesmo mapeamento ativo', () => {
    ...
    expect(core).toContain("if (page === '/oportunidades') return 'opportunities';");
    expect(core).toContain("'/compra-venda'");
    expect(shell).toContain("if (page === '/oportunidades') return 'opportunities';");
    expect(shell).toContain("'/compra-venda'");
