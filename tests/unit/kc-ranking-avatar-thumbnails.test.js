...
// 2026-09-04: /api/og-image substitui /render/image (quota Supabase). jsdom
// resolve a URL relativa contra http://localhost.
const SITE_ORIGIN = 'http://localhost';
const THUMBNAIL = SITE_ORIGIN + '/api/og-image?path=' + encodeURIComponent('kino-media/profile-avatars/test-user/avatar.jpg') + '&w=144&h=144&fit=cover&q=80';
...
describe('ranking avatar thumbnails', () => {
  ...
  test.each(['jpg', 'jpeg', 'png', 'webp', 'JPG'])('supports known raster extension %s and preserves encoded file names', (extension) => {
    ...
    expect(render('renderHomeRanking', source).src).toBe(SITE_ORIGIN + '/api/og-image?path=' + encodeURIComponent(source.replace(ORIGIN + '/storage/v1/object/public/', '')) + '&w=144&h=144&fit=cover&q=80');
  });
