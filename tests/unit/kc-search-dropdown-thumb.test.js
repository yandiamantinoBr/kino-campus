...
describe('search dropdown thumbs', () => {
  ...
  test('converte storage público do Supabase em /api/og-image compacto', () => {
    ...
    expect(thumb).toContain('/api/og-image?path=kino-media%2Fpost-media%2Fu1%2Fp1%2Fcover.jpg');
  ...
  test('converte URLs ja renderizadas (/render/image) em /api/og-image', () => {
    ...
    expect(thumb).toContain('/api/og-image?path=kino-media%2Fpost-media%2Fu2%2Fp2%2Fcapa.png');
  ...
  test('cria thumb com img otimizada e fallback emoji sem imagem', () => {
    ...
    expect(withImage.childNodes[0].src).toContain('/api/og-image');
