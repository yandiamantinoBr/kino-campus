...
test.describe('direitos de privacidade na Central de Ajuda', () => {
  ...
  test('card de Configurações permanece dentro do layout e sem rolagem horizontal no mobile', async ({ page }) => {
    ...
    expect(metrics.copyFallbackHref).toBe('/ajuda?request=data_access_copy#helpRequestForm');
    expect(metrics.portabilityFallbackHref).toBe('/ajuda?request=data_portability#helpRequestForm');
    expect(metrics.deletionFallbackHref).toBe('/ajuda?request=account_erasure#helpRequestForm');
  });
