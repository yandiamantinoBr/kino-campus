...
describe('v67.0.0 - icones decorativos em moderacao admin', () => {
  test('template admin de moderacao esconde icones em feedback de salvar e selecao de usuario', () => {
    const moderation = _fs.readFileSync(_path.join(_ROOT, 'assets/js/controllers/admin/admin-moderation.controller.js'), 'utf8');
    const markup = _fs.readFileSync(_path.join(_ROOT, 'admin/moderation.html'), 'utf8');

    expect(moderation).toContain("button.innerHTML = '<i class=\"fas fa-spinner fa-spin\" aria-hidden=\"true\"></i> ' + escape(label || 'Carregando…')");
    expect(markup).toContain('<i class="fas fa-save" aria-hidden="true"></i> Salvar limite global');
    expect(markup).toContain('<i class="fas fa-save" aria-hidden="true"></i> Salvar');
