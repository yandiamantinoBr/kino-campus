...
page = replaceOnce(
  ...
  '} from "./decks";\nimport {\n  clampSlide,\n  reconcilePresenterSlide,\n  type PendingLocalNavigation,\n} from "./presentation-sync";\n\ntype SessionSnapshot = {',
  'presentation sync import',
);
...
page = replaceOnce(
  ...
  'slide state update type',
);

page = replaceOnce(
  page,
  '  onCurrent: (index: number) => void;',
  '  onCurrent: (update: SlideStateUpdate) => void;',
  'PresenterView onCurrent type',
);
...
page = replaceOnce(
  ...
  '  const next = useCallback(() => onCurrent((value) => Math.min(deck.length - 1, value + 1)), [deck.length, onCurrent]);\n  const previous = useCallback(() => onCurrent((value) => Math.max(0, value - 1)), [onCurrent]);',
  'functional presenter navigation',
);
...
page = replaceOnce(
  ...
  'projection control icon and labeling',
);
...
page = replaceOnce(
  ...
  'session creation initializes local target',
);
...
if (syncStart === -1 || syncEnd === -1) {
  throw new Error('Unable to locate presentation synchronization effects');
}
  ...
  useEffect(() => {
    ...
    const synchronize = async () => {
      try {
        ...
        if (pendingNavigationRef.current?.slide === acknowledged) {
          pendingNavigationRef.current = null;
        }
      } catch {
        // Keep local navigation responsive. Polling reconciles once the service recovers.
      }
    };
    void synchronize();
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        setSession(data.session);
        const reconciliation = reconcilePresenterSlide({
        ...
        pendingNavigationRef.current = reconciliation.pending;
        if (reconciliation.slide !== currentRef.current) {
          currentRef.current = reconciliation.slide;
          setCurrent(reconciliation.slide);
        }
      } catch {
        // The stage remains locally usable while the live service is unavailable.
      } finally {
        inFlight = false;
      }
    };
...
page = replaceOnce(
  ...
  '  if (view === "deck") return <PresenterView duration={duration} mode={mode} deck={deck} current={current} onCurrent={updatePresenterCurrent} onExit={exit} session={session} presenterToken={presenterToken} onCreateSession={createSession} sessionError={sessionError} />;',
  'PresenterView stable updater binding',
);
...
page = replaceOnce(
  ...
  '  return <LaunchScreen duration={duration} mode={mode} onDuration={(value) => { setDuration(value); currentRef.current = 0; pendingNavigationRef.current = null; setCurrent(0); }} onMode={(value) => { setMode(value); currentRef.current = 0; pendingNavigationRef.current = null; setCurrent(0); }} onStart={start} />;',
  'launch selection reset',
);
...
export function reconcilePresenterSlide({
  ...
  if (!pending) {
    return {
      slide: safeRemote,
      pending: null,
      source: safeRemote === safeLocal ? "unchanged" : "remote",
    };
  }
  ...
  if (age <= LOCAL_NAVIGATION_GRACE_MS) {
    return {
      slide: safeLocal,
      pending: { ...pending, slide: safePending },
...
css = replaceOnce(
  ...
  '.presentation-shell { width: 100%; height: 100vh; height: 100dvh; min-height: min(620px, 100dvh); position: relative; overflow: hidden; overscroll-behavior: contain; background: var(--cream); }',
  'dynamic presentation viewport',
);
css = replaceOnce(
  css,
  ...
  'navigation touch target behavior',
);
css = replaceOnce(
  css,
  '  .presentation-shell { min-height: 100dvh; height: auto; overflow: auto; }',
  '  .presentation-shell { width: 100%; min-height: 100dvh; height: auto; overflow: auto; touch-action: pan-y pinch-zoom; }',
...
css = replaceOnce(
  ...
  'mobile slide control clearance',
);
css = replaceOnce(
  css,
  ...
  '  .presentation-controls { position: fixed; left: 50%; right: auto; bottom: max(12px, env(safe-area-inset-bottom)); transform: translateX(-50%); padding: 6px; border-radius: 999px; background: rgba(251,249,245,.92); backdrop-filter: blur(14px); box-shadow: 0 10px 28px rgba(7,26,61,.12); }\n  .presentation-controls button { width: 44px; height: 44px; }',
  'mobile safe-area controls',
);
css = replaceOnce(
  css,
  '  .presentation-shell.is-projection .slide { grid-template-columns: 1fr; }.presentation-shell.is-projection .slide__visual { min-height: 430px; }',
  '  .presentation-shell.is-projection .slide { grid-template-columns: 1fr; }\n  .presentation-shell.is-projection .slide__copy h1 { font-size: clamp(2.8rem, 10.8vw, 4.7rem); }\n  .presentation-shell.is-projection .slide__body { font-size: clamp(1.03rem, 3.8vw, 1.25rem); line-height: 1.55; }\n  .presentation-shell.is-projection .slide__visual { min-height: 430px; }',
  'mobile projection legibility',
);
write(cssPath, css);
...
test("buildDeck preserves all six presentation variants", () => {
  ...
  for (const [key, count] of expected) {
    const [duration, mode] = key.split("-");
    assert.equal(buildDeck(Number(duration), mode).length, count, key);
  }
});

test("local slide changes are not overwritten by a stale poll", () => {
  const {
    LOCAL_NAVIGATION_GRACE_MS,
    reconcilePresenterSlide,
  } = loadTypeScriptModule("app/presentation-sync.ts");
  const pending = { slide: 4, startedAt: 1000 };

  const stale = reconcilePresenterSlide({
    localSlide: 4,
    remoteSlide: 3,
    totalSlides: 20,
    pending,
    now: 1000 + LOCAL_NAVIGATION_GRACE_MS - 1,
  });
  assert.equal(stale.slide, 4);
  assert.equal(stale.source, "local-pending");
  assert.equal(stale.pending.slide, 4);

  const acknowledged = reconcilePresenterSlide({
    localSlide: 4,
    remoteSlide: 4,
    totalSlides: 20,
    pending,
  ...
  assert.equal(remoteAfterTimeout.pending, null);
});

test("presenter controls use stable navigation, guarded gestures and a zoom icon", () => {
  const page = read("app/page.tsx");
  assert.match(page, /onCurrent\(\(value\) => Math\.min\(deck\.length - 1, value \+ 1\)\)/);
  assert.match(page, /event\.repeat && navigationKeys\.includes\(event\.key\)/);
  assert.match(page, /Math\.abs\(deltaX\) <= Math\.abs\(deltaY\) \* 1\.25/);
  assert.match(page, /onTouchCancel=\{\(\) => \{ touchStart\.current = null; \}\}/);
  assert.match(page, /<ZoomIn size=\{18\} \/>/);
  assert.match(page, /Ampliar conteúdo para projeção/);
  assert.doesNotMatch(page, /title="Aumentar a legibilidade na projeção"><Accessibility/);
  assert.match(page, /reconcilePresenterSlide/);
  assert.match(page, /pendingNavigationRef/);
});

test("mobile stage reserves safe-area space for fixed navigation", () => {
  const css = read("app/globals.css");
  assert.match(css, /height: 100dvh/);
  assert.match(css, /bottom: max\(12px, env\(safe-area-inset-bottom\)\)/);
  assert.match(css, /padding: 45px 24px calc\(106px \+ env\(safe-area-inset-bottom\)\)/);
