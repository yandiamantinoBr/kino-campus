# add-posts-read-script-tag.ps1
# Insere supabase.posts-read.adapter.js antes de supabase.saved.adapter.js em todos os HTMLs
...
foreach ($f in $rootFiles) {
  ...
  if (Test-Path $path) {
    ...
    if ($content -notmatch [regex]::Escape("supabase.posts-read.adapter.js")) {
      $pattern = '(<script defer src="assets/js/adapters/supabase\.saved\.adapter\.js"></script>)'
      $replacement = '<script defer src="assets/js/adapters/supabase.posts-read.adapter.js"></script>' + "`r`n    " + '$1'
