# add-posts-write-script-tag.ps1
# Insere supabase.posts-write.adapter.js antes de supabase.adapter.js em todos os HTMLs
...
foreach ($f in $rootFiles) {
  ...
  if (Test-Path $path) {
    ...
    if ($content -notmatch [regex]::Escape("supabase.posts-write.adapter.js")) {
      $pattern = '(<script defer src="assets/js/adapters/supabase\.adapter\.js"></script>)'
      $replacement = '<script defer src="assets/js/adapters/supabase.posts-write.adapter.js"></script>' + "`r`n  " + '$1'
